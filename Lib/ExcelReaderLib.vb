...
' ============================================================================
' ExcelReaderLib - Excel File Reading for Element Configuration
' 
' Provides functions to read element tables from Excel files.
' Uses Excel COM Interop (requires Excel to be installed).
'
' Terminology updated 2026-05-12: variant/moodul -> element per UBIQUITOUS_LANGUAGE.md
'
' Usage: AddVbFile "Lib/ExcelReaderLib.vb"
'
' Excel Table Format (new naming):
' | ElementiNimi | Param1 | Param2 | ... |
' |--------------|--------|--------|-----|
' | Element-A    | 100    | 200    | ... |
' | Element-B    | 150    | 250    | ... |
'
' Legacy naming (MooduliNimi) also supported for backward compatibility.
'
' First row is headers (parameter names)
' First column is ElementiNimi/MooduliNimi (used for folder naming)
' Remaining columns are parameter names and values
' PartNumber is auto-generated by Vault numbering scheme, not specified in Excel
'
...
Public Module ExcelReaderLib
    ...
    ''' <summary>
    ''' Represents a single element configuration read from Excel.
    ''' </summary>
    ...
    ''' Returns a list of ElementConfig objects.
    ''' Supports both ElementiNimi (new) and MooduliNimi (legacy) column names.
    ''' </summary>
    ...
    ''' <returns>List of element configurations</returns>
    Public Function ReadVariantTable(excelPath As String, Optional sheetName As String = "") As List(Of ElementConfig)
        Dim configs As New List(Of ElementConfig)
        ...
        Try
            ...
            If rowCount < 2 OrElse colCount < 1 Then
                Throw New Exception("Excel table must have at least 2 rows (header + data) and 1 column (ElementiNimi)")
            End If
            ...
            For row As Integer = 2 To rowCount
                Dim cfg As New ElementConfig()
                
                ' First column is ElementiNimi/MooduliNimi (stored as ElementName)
                Dim elementName As Object = worksheet.Cells(row, 1).Value
                If elementName Is Nothing OrElse String.IsNullOrWhiteSpace(elementName.ToString()) Then
                    Continue For ' Skip empty rows
                End If
                cfg.ElementName = elementName.ToString().Trim()
                
                ' PartNumber is auto-generated, not from Excel
                cfg.PartNumber = ""
                
                ' Remaining columns (2+) are parameters
                For col As Integer = 2 To colCount
    ...
    ''' </summary>
    Public Function GetElementByName(configs As List(Of ElementConfig), elementName As String) As ElementConfig
        For Each cfg As ElementConfig In configs
            If cfg.ElementName.Equals(elementName, StringComparison.OrdinalIgnoreCase) Then
                Return cfg
            End If
        Next
        Return Nothing
    ...
    ''' <summary>
    ''' Get list of element names from the list.
    ''' </summary>
    Public Function GetElementNames(configs As List(Of ElementConfig)) As List(Of String)
        Dim names As New List(Of String)
        For Each cfg As ElementConfig In configs
            names.Add(cfg.ElementName)
        Next
        Return names
    End Function
    ...
    ''' </summary>
    Public Function ValidateParameters(doc As Inventor.Document, cfg As ElementConfig) As List(Of String)
    ...
    ''' Returns the selected ElementConfig, or Nothing if cancelled.
    ''' </summary>
    Public Function ShowConfigSelectionDialog(configs As List(Of ElementConfig)) As ElementConfig
        ...
        lst.Height = 180
        For Each cfg As ElementConfig In configs
            lst.Items.Add(cfg.ElementName & " (" & cfg.PartNumber & ")")
        Next
